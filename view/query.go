...
import (
	...
	"goravel/packages/tinker/styles"
	"goravel/packages/tinker/symbols"
	"log"
	"os"
	"os/exec"
	"path/filepath"
...
func Rebuild() {
	...
	cmd.Dir = path.Base("packages/tinker") // 设置工作目录为包含 Makefile 的目录
...
func StartYaegiModel() {
	...
	for {
		...
		if input == "shebang" {
			...
			preStr := `import "goravel/app/models"`
